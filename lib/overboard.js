...
ob.build = function(jsonString) {
	...
	var report = new ob.Capsize(json.title, json.message, json.status, json.error, json.source);
	report.setLink(json.links.self, json.links.about);
	report.setMeta(json.meta);
...
ob.Capsize = class {
	...
	save() {
		...
		fs.appendFile('./overboard.log', this.toString() + "\n", (err) => {
			if (err) {
				log.log(log.overboard, "save", err);
				return;
			}

			log.log(log.overboard, "save", "Report saved to overboard.log");
		});
	}
